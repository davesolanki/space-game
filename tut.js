...
function preload() {
    ...
    game.load.image('boss', 'assets/boss2.png');
    game.load.image('boss2', 'assets/boss3.png');
    game.load.image('deathRay', 'assets/death-ray.png');

    game.load.spritesheet('explosion', 'assets/explode.png', 128, 128);

    game.load.audio('bossmusic', 'assets/bosstrack.ogg')
...
function create() {
    ...
    bossmusic.addMarker('boss', 4, 18, 1, true);
    boss = game.add.sprite(0, 0, 'boss');
    boss.exists = false;
    boss.alive = false;
    boss.anchor.setTo(0.5, 0.5);
    // if (boss_num*40 > 50) {
    //     boss.damageAmount = boss_num*40;
    // } else {
...
function addRay(leftRight) {
        var ray = game.add.sprite(leftRight * boss.width * 0.5, 0, 'deathRay');
        ray.alive = false;
        ray.visible = false;
        boss.addChild(ray);
        ...
        ray.body.setSize(ray.width / boss_num + 2, ray.height / 4);
        ray.update = function() {
    ...
    level_num = ['LEVEL 1', 'LEVEL 2', 'LEVEL 3', 'LEVEL 4', 'LEVEL 5']
	level = game.add.bitmapText(game.world.centerX, game.world.centerY, 'spacefont', level_num[0], 110);
    level.x = level.x - level.textWidth / 2;
    level.y = level.y - level.textHeight / 3;
    level.visible = false;

    level2 = game.add.bitmapText(game.world.centerX, game.world.centerY, 'spacefont', level_num[1], 110);
    level2.x = level2.x - level2.textWidth / 2;
    level2.y = level2.y - level2.textHeight / 3;
    level2.visible = false;

    level3 = game.add.bitmapText(game.world.centerX, game.world.centerY, 'spacefont', level_num[2], 110);
    level3.x = level3.x - level3.textWidth / 2;
    level3.y = level3.y - level3.textHeight / 3;
    level3.visible = false;

    level4 = game.add.bitmapText(game.world.centerX, game.world.centerY, 'spacefont', level_num[3], 110);
    level4.x = level4.x - level4.textWidth / 2;
    level4.y = level4.y - level4.textHeight / 3;
...
function launchBoss() {
    ...
    boss.health = (boss_num*500)+1;
    // console.log(boss.damageAmount);
...
function new_level1() {
	level.visible = true;
    level.alpha = 0;
    var fadeInLevel = game.add.tween(level);
    fadeInLevel.to({alpha: 1}, 1000, Phaser.Easing.Quintic.Out);
    fadeInLevel.start();
    game.time.events.add(2000, function() {
    game.add.tween(level).to({y: 0}, 1500, Phaser.Easing.Linear.None, true);
    game.add.tween(level).to({alpha: 0}, 1500, Phaser.Easing.Linear.None, true);
}, this);
}
...
function new_level2() {
	level2.visible = true;
    level2.alpha = 0;
    var fadeInLevel2 = game.add.tween(level2);
    fadeInLevel2.to({alpha: 1}, 1000, Phaser.Easing.Quintic.Out);
    fadeInLevel2.start();
    game.time.events.add(2000, function() {
    game.add.tween(level2).to({y: 0}, 1500, Phaser.Easing.Linear.None, true);
    game.add.tween(level2).to({alpha: 0}, 1500, Phaser.Easing.Linear.None, true);
}, this);
}
...
function new_level3() {
	level3.visible = true;
    level3.alpha = 0;
    var fadeInLevel3 = game.add.tween(level3);
    fadeInLevel3.to({alpha: 1}, 1000, Phaser.Easing.Quintic.Out);
    fadeInLevel3.start();
    game.time.events.add(2000, function() {
    game.add.tween(level3).to({y: 0}, 1500, Phaser.Easing.Linear.None, true);
    game.add.tween(level3).to({alpha: 0}, 1500, Phaser.Easing.Linear.None, true);
}, this);
}
...
function new_level4() {
	level4.visible = true;
    level4.alpha = 0;
    var fadeInLevel4 = game.add.tween(level4);
    fadeInLevel4.to({alpha: 1}, 1000, Phaser.Easing.Quintic.Out);
    fadeInLevel4.start();
    game.time.events.add(2000, function() {
    game.add.tween(level4).to({y: 0}, 1500, Phaser.Easing.Linear.None, true);
    game.add.tween(level4).to({alpha: 0}, 1500, Phaser.Easing.Linear.None, true);
}, this);
}
